    ...
    public function dataPagination(Request $request)
    {
        ...
        try {
            ...
            Loop::run(function () use ($jumlah, $offset, &$data, &$totalData) {
                $promise = [];
                $promise['data'] = \Amp\call(function () use ($jumlah, $offset) {
                    return ModelArticle::with([
                        'article_attachment' => function ($query) use ($jumlah, $offset) {
                            $query->limit($jumlah);
                        },
                        'article_category'
                    ])->skip($offset)->take($jumlah)->get();
                });
                $promise['jumlahData'] = \Amp\call(function () {
